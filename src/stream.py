...
def say(_message):
    url = (
        "https://tts.voicetech.yandex.net/tts?&format=mp3&lang=ru_RU&speed=1.3&emotion=good&speaker=ermil_with_tuning&robot=1&text="
        + _message
    )
...
def insert_word(original_string, new_word):
    ...
    new_string = (
        original_string[:random_boundary] + new_word + original_string[random_boundary:]
    )
...
def handle_message(message):
    ...
    if (
        ...
        response = openai.Completion.create(
            ...
            prompt='give a new short and funny answer to this message: "'
            + decoded_message
            + '"',
            temperature=0.75,
            max_tokens=256,
        ...
        print("[text_ai][first_response]: " + Color.WARNING + ai_message + Color.ENDC)
        ...
        print(
            "[text_ai][second_response]: " + Colors.OKGREEN + ai_message + Colors.ENDC
        )
        say(ai_message)
    else:
        print("[text_ai]: " + Colors.WARNING + str(decoded_message) + Colors.ENDC)
